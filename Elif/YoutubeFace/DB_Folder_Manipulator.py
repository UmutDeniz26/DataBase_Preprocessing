...
logFolderPath = './Elif/LOG/'+ dbName
dbName = './Elif/'+dbName
txtInfoPath = './Elif/youtubeFaceDB.txt' #only for imgTxtDBs
...
if dbName == './Elif/YoutubeFace' or dbName == './Elif/LFW':
    imgTxtDBs = True
else:
    imgTxtDBs = False
...
def yunetDetectionDNN(img,img_path):
    ...
    detector = cv2.FaceDetectorYN.create("./Elif/face_detection_yunet.onnx",  "", (0, 0))
            ...
            for inner_file in innerFolder:
                ...
                if folder_flag == True:
                    ...
                    for inner_inner_file in inner_inner_folder:
                        if len(os.listdir('./'+dbName+'/'+file_name+'/'+inner_file.name)[0].split('.')) == 1:
                            inner_inner_inner_folder = os.scandir('./'+dbName+'/'+file_name+'/'+inner_file.name+'/'+inner_inner_file.name)
                            for inner_inner_inner_file in inner_inner_inner_folder:
                                folder_flag = len(inner_inner_file.name.split('.')) == 1
                                if inner_inner_inner_file.name.split('.')[-1] == "jpg":    
                                    outFilesPaths.append('./'+dbName+'/'+file_name+'/'+inner_file.name+'/'+inner_inner_file.name+'/'+inner_inner_inner_file.name)
                        folder_flag = len(inner_inner_file.name.split('.')) == 1
